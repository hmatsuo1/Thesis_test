...
plt.plot(gdp_indexed.index, gdp_indexed[treated], label='UK (Actual)', linewidth=2)
plt.plot(gdp_indexed.index, synthetic, label='Synthetic UK', linestyle='--')
...
# SCM効果の差（ギャップ）
scm_gap = gdp_indexed[treated] - synthetic
plt.figure(figsize=(10, 4))
plt.plot(gdp_indexed.index, scm_gap, label='Gap (UK - Synthetic)', color='red')
...
# 表示：各国への学習で得た重み
pd.Series(weights, index=donors).sort_values(ascending=False)
